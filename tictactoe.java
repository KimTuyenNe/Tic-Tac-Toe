...
class Board{
	...
	public void placeAMove(Point point , int player){
		board[point.x][point.y] = player;    // player == 1 for x ,2 for 0
	}
	...
	public int minimax(int depth , int turn){
		...
		for (int i = 0; i < pointsAvailable.size() ;++i){
			...
			if (turn == 1){ //X's turn select select the highest from below minimax() call
			placeAMove(point, 1);
			...
			else if (turn == 2 ){ //O's turn select select the highest from below minimax() call
				placeAMove(point , 2);
...
public class tictactoe{
	public static void main(String[] args) {
		...
		System.out.println("Who is gonna move first ? (1) Computer (2)User");
